...
/**
 * A static data store of [Account]s.
 */
object LocalAccountsDataProvider {
    val allAccounts = listOf(
        Account(
            id = 0L,
            firstName = R.string.account_0_first_name,
            lastName = R.string.account_0_last_name,
            email = R.string.account_0_email,
        ...
        Account(
            id = 2L,
            firstName = R.string.account_2_first_name,
            lastName = R.string.account_2_last_name,
            email = R.string.account_2_email,
            avatar = R.drawable.avatar_2
        ),
        Account(
            id = 3L,
            firstName = R.string.account_3_first_name,
            lastName = R.string.account_3_last_name,
            email = R.string.account_3_email,
    ...
    /**
     * The default account to use as the current user.
     */
    val defaultAccount: Account = allAccounts[0]

    /**
     * Get an [Account] with the given [id].
     */
    fun getContactAccountById(id: Long): Account {
        return allAccounts.first { it.id == id }
    }
}
